...
class BattalionConstruction:
    ...
    def DeployBattalion(self, defendingline, construction, army, kind, number, placetype, linespercell = -1, maxpercell = -1, command = Command.DEFEND_CASTLE):
        ...
        if (not Battalion.battalionFitsInCell(kind, self._defendingLines[defendingline].GetCellSize(), self._defendingLines[defendingline].GetCellSize())):
            Log('Too big battalion units (%s) to be deployed in this wall' % (battalion.__class__.__name__), VERBOSE_WARNING)
            return False

        # Calculates the maximum battalion size that fits into a cell
        sizex = math.ceil(self._battalionGridCellSize / battalion.GetBounding().length)
        #sizey = math.ceil(self._lineWidth / battalion.GetBounding().width)
        sizey = sizex
        ...
        if (placetype == CONSTRUCTION_BATTALION_DEPLOYMENT_CONSECUTIVE):
            ...
            while ((avaiable > 0) and not end):
                ...
                if (factory.IsArcher(b)):
                    margin = Battles.Utils.Settings.SETTINGS.Get_F('Army', 'Archers', 'DefendersMarginSpace')
                else:
                    margin = 0


                if (self._defendingLines[defendingline].AppendBattalion(b, offset = margin)):
            ...
            Log( 'Placed wall battalions (%s troops: %d). Remain troops: %d' % (battalion.GetLabel(), battalion.GetNumber() - avaiable, avaiable))
            ...
            # Number of avaiable cells
            cells = battalion.GetNumber() / size
            ...
            while ((avaiable > 0) and not end):
                ...
                if (init):
                    success = self._defendingLines[defendingline].AppendBattalion(b, offset = margin)
                    init = False
                else:
                    success = self._defendingLines[defendingline].AppendBattalion(b, offset = factor + margin)
            ...
            Log( 'Placed wall battalions (troops: %d). Remain troops: %d' % (battalion.GetNumber() - avaiable, avaiable))
        return True
    # ########################################################################## OLD CODE END
...
#         #   maxpercell: For each grid cell, the maximum number of soldiers per battalion. -1 means no limits
# 
#         #print "###################################### Function DeployBattalion:", kind, number, "########################################"
#         #factory = ArmyFactory()
#         #battalion = None
...
#         if (placetype == CONSTRUCTION_BATTALION_DEPLOYMENT_CONSECUTIVE):
#             #avaiable = battalion.GetNumber()
#             available = army.getBattalionFreeNumber(kind)
#             end = False
#             while ((available > 0) and not end):
# 
#                 if (available > size):
#                     n = size
#                 else:
...
# 
# 
#             Log( 'Placed wall battalions (%s troops: %d). Remain troops: %d' % (kind, army.getBattalionFreeNumber(kind) - available, available))
# 
# 
# 
#         elif (placetype == CONSTRUCTION_BATTALION_DEPLOYMENT_SPARSED):
